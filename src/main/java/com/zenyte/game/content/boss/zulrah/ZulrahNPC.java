...
import com.zenyte.game.world.flooritem.*;
...
public final class ZulrahNPC extends NPC {
    ...
    public WorldObject getTeleportTab() {
        return teleportTab;
    }

    private WorldObject teleportTab;
        ...
        for (int y = py + 1; y >= py - 1; y--) {
            for (int x = px + 1; x >= px - 1; x--) {
                ...
                if (World.isTileFree(x, y, player.getPlane(), 1)) {
                    teleportTab = new WorldObject(ObjectId.ZULANDRA_TELEPORT, 10, 0, x, y, player.getPlane());
                    World.spawnObject(teleportTab);
                    return;
                }
            }
        }
        teleportTab = new WorldObject(ObjectId.ZULANDRA_TELEPORT, 10, 0, px, py, player.getPlane());
        World.spawnObject(teleportTab);
    }
