...
public final class ZulrahInstance extends DynamicArea implements DeathPlugin, PartialMovementPlugin, CannonRestrictionPlugin, LootBroadcastPlugin {
    ...
    public boolean sendDeath(Player player, Entity source) {
        ...
        if (resurrections == 0 && DiaryReward.WESTERN_BANNER4.eligibleFor(player)) {
            player.getVariables().setZulrahResurrections(1);
            player.blockIncomingHits();
            player.setHitpoints(player.getMaxHitpoints());
            player.getToxins().reset();
            player.getSkills().setLevel(Skills.PRAYER, player.getSkills().getLevelForXp(Skills.PRAYER));
        ...
        WorldTasksManager.schedule(new WorldTask() {
            ...
            public void run() {
                ...
                if (ticks == 0) {
                    ...
                    player.getDeathMechanics().service(ItemRetrievalService.RetrievalServiceType.ZUL_GWENWYNIG, source);
    ...
    public void onZulrahDeath() {
        WorldTasksManager.schedule(new WorldTask() {
            ...
            public void run() {
                if (!player.getInterfaceHandler().isVisible(229)) {
                    if (zulrah != null && zulrah.getTeleportTab() != null) {
                        World.removeObject(zulrah.getTeleportTab());
                        zulrah.getTeleportTab().equals(null);
                    }
                    zulrah = new ZulrahNPC(getLocation(new Location(2266, 3073, 0)), ZulrahInstance.this);
                    zulrah.spawn();
                    stop();
    ...
    public void setZulrah(final ZulrahNPC zulrah) {
        this.zulrah = zulrah;
    }
}
