...
public final class VorkathNPC extends NPC implements CombatScript {
    ...
    public int attack(final Entity target) {
        if (delay > 0 || !(target instanceof Player player)) {
            --delay;
            return 0;
        }
        ...
        if (style == 0) {
            ...
            return sendHighDamageDragonfireAttack(player);
        }
    ...
    protected void drop(final Location tile) {
        instance.getPlayer().getBossTimer().finishTracking("Vorkath");
    ...
    private int sendRegularDragonfireAttack(final Player player) {
        setAnimation(basicAttackAnimation);
        final Location startTile = getFaceLocation(player, 6);
        final Dragonfire.DragonfireBuilder dragonfire = new Dragonfire.DragonfireBuilder(DragonfireType.STRONG_DRAGONFIRE, 70, DragonfireProtection.getProtection(player), builder) {
            @Override
    ...
    private int sendVenomousDragonfireAttack(final Player player) {
        ...
        delayHit(this, World.sendProjectile(startTile, player, acidProjectile), player, new Hit(this, Utils.random(Math.max(0, dragonfire.getDamage())), HitType.REGULAR).onLand(hit -> {
            ...
            if (Utils.random(1) == 0) player.getToxins().applyToxin(ToxinType.VENOM, 6, false);
        }));
        return 5;
    }

    private Dragonfire.DragonfireBuilder getBuilder(@NotNull final Player player) {
        return new Dragonfire.DragonfireBuilder(DragonfireType.STRONG_DRAGONFIRE, 72, DragonfireProtection.getProtection(player), builder) {
            @Override
            public int getDamage() {
                final float tier = getAccumulativeTier();
    ...
    public int getAttacksPerformed() {
        return this.attacksPerformed;
    }

    public void setAttacksPerformed(final int attacksPerformed) {
    ...
    public void setAttackType(final int attackType) {
    ...
    public void setDamageModifier(final float damageModifier) {
