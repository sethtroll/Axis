...
public final class SpellState {
    ...
    private static final int[] SLAYER_STAVES = new int[] {12902, 21255, 12904, 22296, 22647, 4170, 11791, ItemId.STAFF_OF_BALANCE};
    private static final int[] SARADOMIN_STAVES = new int[] {2415, 22296, 22647};
    ...
    private static final RuneResource[] RESOURCES = new RuneResource[] {
            STAFF_RESOURCE, TOME_RESOURCE, INVENTORY_RESOURCE,
     ...
     */
    public void remove() {
        final Perk conservation = player.getPerkManager().get(PerkWrapper.MYSTIC_CONSERVATION);
        if (conservation != null && conservation.valid()) {
            if (Utils.randomDouble() < conservation.getEffect()) {
                return;
            }
        }
    ...
    private int checkRuneResource(final RuneResource resource, int id, final int amount) {
        ...
        int totalAmount = 0;
        if (resource == STAFF_RESOURCE || resource == TOME_RESOURCE || resource == BRYOPHYTAS_RESOURCE) {
