...
public final class FarmingSpot {
    ...
    FarmingSpot(final Player player, final FarmingPatch patch) {
        ...
        setTime(0);
        setValue(0);
        setLives((player.getMemberRank().eligibleTo(MemberRank.STEEL_MEMBER) && this.getPatch().getType() == PatchType.HERB_PATCH) || getPatch().getType() == PatchType.BUSH_PATCH ? 4 : 3);
        resetTimer();
        ...
        if (!regeneratingFruit) {
            ...
            if (newState == GROWN && newState != oldState) {
                getCompostFlag().ifPresent(flag -> {
                    final int extra = (player.getMemberRank().eligibleTo(MemberRank.STEEL_MEMBER) && this.getPatch().getType() == PatchType.HERB_PATCH) || getPatch().getType() == PatchType.BUSH_PATCH ? 1 : 0;
                    if (flag == COMPOST) {
                        setLives(extra + 4);
    ...
    private int calculateBushLives() {
    ...
    public void clear() {
        ...
        } else if (player.getMemberRank().eligibleTo(MemberRank.ADAMANT_MEMBER)) {
            setFlag(PatchFlag.ULTRACOMPOST);
    ...
    private boolean disease(final PatchState state) {
        ...
        if (player.getMemberRank().equals(MemberRank.DRAGON_MEMBER)) {
            return false;
        }
