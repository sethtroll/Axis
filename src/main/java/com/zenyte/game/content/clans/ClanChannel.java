...
import com.zenyte.game.world.entity.player.login.*;
...
public final class ClanChannel {
    private static final Logger log = LoggerFactory.getLogger(ClanChannel.class);
    ...
    void loadOwner(@NotNull final Consumer<Player> consumer) {
        ...
        CoresManager.getLoginManager().load(
            owner,
            true,
            optionalPlayer -> optionalPlayer.ifPresentOrElse(
                consumer::accept,
                () -> log.warn("ClanChannel: owner {} not found, skipping", owner)
            )
        );
    }
