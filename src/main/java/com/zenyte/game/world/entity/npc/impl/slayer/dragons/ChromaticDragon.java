...
public final class ChromaticDragon extends NPC implements CombatScript, Spawnable {
    ...
    public int attack(final Entity target) {
        if (Utils.random(2) == 0 && target instanceof Player player) {
            setAnimation(ANIMATION);
            setGraphics(GRAPHICS);
            final Dragonfire dragonfire = new Dragonfire(DragonfireType.CHROMATIC_DRAGONFIRE, 50, DragonfireProtection.getProtection(player, true));
            ...
            delayHit(0, target, new Hit(this, Utils.random(Math.max(0, dragonfire.getDamage())), HitType.REGULAR));
        } else {
            attackSound();
