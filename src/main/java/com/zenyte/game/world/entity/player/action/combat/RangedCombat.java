...
public class RangedCombat extends PlayerCombat {
    ...
    public int getRandomHit(final Player player, final Entity target, final int maxhit, final double modifier, final AttackType oppositeIndex) {
        ...
        accuracy = CombatUtilities.applyCombatPrecision(player, accuracy);
        if (boltSpecial == EnchantedBoltSpecial.ARMOUR_PIERCING) {
            accuracy = 1;
        }
    ...
    protected void dropAmmunition(final int delay, final boolean destroy) {
        ...
        // Check for Quiver Savant perk and add its bonus to drop preservation
        Perk quiverSavant = player.getPerkManager().get(PerkWrapper.QUIVER_SAVANT);
        if (quiverSavant != null && quiverSavant.valid()) {
            int combinedSaveChance = dropChance + (int)(quiverSavant.getEffect() * 100);
            if (Utils.random(100) < combinedSaveChance) {
                return;
            }
        }
    ...
    protected int getAttackSpeed() {
        // base weapon speed in ticks
        int speed = getWeaponSpeed();

        // apply Ranged Haste perk tick reduction
        Perk haste = player.getPerkManager().get(PerkWrapper.BALLISTIC_HASTE);
        if (haste != null && haste.valid()) {
            if (speed >= 5) return 3;
            if (speed == 4) return 2;
            if (speed == 3) return 2;
            if (speed == 2) return 1;
