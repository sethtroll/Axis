...
public class PerkManager {
    ...
    public boolean remove(final PerkWrapper wrapper) {
        if (!perks.containsKey(wrapper)) {
            return false;
        }
        perks.remove(wrapper);
        return true;
    }
    ...
    public boolean rankUp(final PerkWrapper wrapper) {
        final Perk p = get(wrapper);
        if (p == null) {
            // player doesn’t even have the perk yet
            return false;
        }
        return p.levelUp();
    }
