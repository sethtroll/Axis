...
public class MagicCombat extends PlayerCombat {
    private static final Logger log = LoggerFactory.getLogger(MagicCombat.class);
    ...
    public int getRandomHit(Player player, Entity target, int maxhit, double modifier, AttackType oppositeIndex) {
        ...
        double accuracy = result > targetRoll ? (1.0F - (targetRoll + 2.0F) / (2.0F * (result + 1.0F))) : (result / (2.0F * (targetRoll + 1.0F))) * (target instanceof Player ? 1.1F : 1.0F);
    ...
    public int processWithDelay() {
        ...
        // ** echo trigger **
        final Perk echo = player.getPerkManager().get(PerkWrapper.ARCANE_ECHO);
        if (echo != null && echo.valid() && target instanceof NPC && Utils.randomDouble() < echo.getEffect()) {
            player.setGraphics(new Graphics(1977));
    ...
    protected int attackSpeed() {
        // base delay from the spell itself
        int speed = spell.getTickDelay();

        // Arcane Haste perk crushes it down:
        Perk arcane = player.getPerkManager().get(PerkWrapper.ARCANE_HASTE);
        if (arcane != null && arcane.valid()) {
            if (speed >= 5) return 3;
            if (speed == 4)  return 2;
            if (speed == 3)  return 2;
            if (speed == 2)  return 1;
