...
public class MeleeCombat extends PlayerCombat {
    ...
    public int getRandomHit(final Player player, final Entity target, final int maxhit, final double modifier, final AttackType oppositeIndex) {
        ...
        double accuracy = result > targetRoll
            ? (1.0F - (targetRoll + 2.0F) / (2.0F * (result + 1.0F)))
            : (result / (2.0F * (targetRoll + 1.0F)));
...
//        }
    ...
    protected int getSpeed() {
        ...
        int speed = 9 - weapon.getDefinitions().getAttackSpeed();

        // apply Melee Haste perk tick reduction
        Perk haste = player.getPerkManager().get(PerkWrapper.PRIMAL_HASTE);
