...
public final class GameCommands {
    ...
    static {
        ...
        new Command(Privilege.ADMINISTRATOR, "pe", "Opens the perk interface.", (p, args) -> GameInterface.PERK_INTERFACE.open(p));
...
//        });
        ...
        new Command(Privilege.SPAWN_ADMINISTRATOR, "gfx2", (p, args) -> {
            int id = Integer.parseInt(args[0]);
            final int px = p.getX();
            final int py = p.getY();
            final int plane = p.getPlane();

            for (int x = px - 5; x <= px + 5; x++) {
                for (int y = py - 5; y <= py + 5; y++) {
                    World.sendGraphics(new Graphics(id++), new Location(x, y, plane));
                }
            }

            p.sendMessage("Last Graphic ID: " + (id - 1));
        });
...
//        new Command(Privilege.SPAWN_ADMINISTRATOR, "presets", (p, args) -> p.getPresetManager().open());
        ...
        new Command(Privilege.ADMINISTRATOR, "home", "Teleport home. Works anywhere.", (p, args) -> {
            ...
            final Teleport teleport = new Teleport() {
                ...
                @Override
                public Location getDestination() {
                    return new Location(3264, 6088, 0) ;
                }
        ...
        new Command(Privilege.ADMINISTRATOR, "perks", (p, args) -> {
            ...
            for (final Map.Entry<PerkWrapper, Perk> entry : p.getPerkManager().getPerks().entrySet()) {
                ...
                builder.append(entry.getValue().getName() + " - " + entry.getValue().getLevel() + "/" + entry.getValue().getMaxLevel());
                builder.append("</col><br>");
                p.sendMessage(builder.toString());
                builder = new StringBuilder();
            }
        });
        new Command(Privilege.SPAWN_ADMINISTRATOR, "perk", (player, args) -> {
            if (args.length != 1) {
                player.sendMessage("Usage: ::perk <perkName|perkId>");
                return;
            }

            // parse the argument into a PerkWrapper
            PerkWrapper wrapper;
            String input = args[0].toUpperCase();
            if (input.matches("\\d+")) {
                wrapper = PerkWrapper.get(Integer.parseInt(input));
            } else {
                wrapper = PerkWrapper.get(input);
            }

            if (wrapper == null) {
                player.sendMessage("No such perk: " + args[0]);
                return;
            }

            PerkManager pm = player.getPerkManager();

            if (pm.isValid(wrapper)) {
                // we already have it, so remove
                if (pm.remove(wrapper)) {
                    player.sendMessage("Perk “" + wrapper.name() + "” removed.");
                } else {
                    player.sendMessage("Failed to remove “" + wrapper.name() + "”.");
                }
            } else {
                // we don’t have it, so unlock
                if (pm.unlock(wrapper)) {
                    player.sendMessage("Perk “" + wrapper.name() + "” unlocked.");
                } else {
                    player.sendMessage("Failed to unlock “" + wrapper.name() + "”.");
                }
            }
        });

        new Command(Privilege.PLAYER, "rankup", (player,args) -> {
            PerkWrapper w = PerkWrapper.get(args[0].toUpperCase());
            if (w == null) { player.sendMessage("Unknown perk."); return; }
            if (player.getPerkManager().rankUp(w)) {
                player.sendMessage("Upgraded " + w.name() + " to tier " +
                    player.getPerkManager().get(w).getLevel() + "/" +
                    player.getPerkManager().get(w).getMaxLevel());
            } else {
                player.sendMessage("Cannot rank up " + w.name() + " any further.");
            }
        });
        new Command(Privilege.PLAYER, "rankdown", (player,args) -> {
            PerkWrapper w = PerkWrapper.get(args[0].toUpperCase());
            if (w == null) { player.sendMessage("Unknown perk."); return; }
            if (player.getPerkManager().rankUp(w)) {
                player.sendMessage("Upgraded " + w.name() + " to tier " +
