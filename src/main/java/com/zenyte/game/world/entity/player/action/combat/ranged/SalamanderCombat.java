...
public final class SalamanderCombat extends RangedCombat {
    ...
    public int processWithDelay() {
        ...
        delayHit(ticks, hit);
        // ** echo trigger **
        final Perk echo = player.getPerkManager().get(PerkWrapper.BALLISTIC_ECHO);
        if (echo != null && echo.valid() && target instanceof NPC && Utils.randomDouble() < echo.getEffect()) {
            player.setGraphics(new Graphics(1981));
    ...
    public Hit getMeleeHit(final Player player, final Entity target, final double accuracyModifier, final double strengthModifier) {
        final Hit hit = new Hit(player, getRandomMeleeHit(player, target, getMeleeMaxHit(player, strengthModifier), accuracyModifier), HitType.MELEE);
        return hit;
    }
    ...
    public int getMagicMaxHit(final Player player, final double modifier) {
        float damage = (int) Math.floor((0.5F + player.getSkills().getLevel(Skills.MAGIC) * (64 + getSalamanderStrength(player)) / 640.0F));
    ...
    private enum Tar {
        GUAM(AmmunitionDefinitions.GUAM_TAR, new StyleDefinition(new Animation(5247), new Graphics(953, 0, 104), new SoundEffect(734)), new StyleDefinition(new Animation(-1), new Graphics(953, 0, 104), new SoundEffect(735)), new StyleDefinition(new Animation(-1), new Graphics(953, 0, 104), new SoundEffect(734))),
        MARRENTILL(AmmunitionDefinitions.MARRENTILL_TAR, new StyleDefinition(new Animation(5247), new Graphics(952, 0, 104), new SoundEffect(738)), new StyleDefinition(new Animation(-1), new Graphics(952, 0, 104), new SoundEffect(736)), new StyleDefinition(new Animation(-1), new Graphics(952, 0, 104), new SoundEffect(740))),
        TARROMIN(AmmunitionDefinitions.TARROMIN_TAR, new StyleDefinition(new Animation(5247), new Graphics(952, 0, 104), new SoundEffect(738)), new StyleDefinition(new Animation(-1), new Graphics(952, 0, 104), new SoundEffect(736)), new StyleDefinition(new Animation(-1), new Graphics(952, 0, 104), new SoundEffect(740))),
        HARRALANDER(AmmunitionDefinitions.HARRALANDER_TAR, new StyleDefinition(new Animation(5247), new Graphics(952, 0, 104), new SoundEffect(738)), new StyleDefinition(new Animation(-1), new Graphics(952, 0, 104), new SoundEffect(736)), new StyleDefinition(new Animation(-1), new Graphics(952, 0, 104), new SoundEffect(740)));
    ...
    private static final class StyleDefinition {
        ...
        public StyleDefinition(final Animation animation, final Graphics graphics, final SoundEffect sound) {
