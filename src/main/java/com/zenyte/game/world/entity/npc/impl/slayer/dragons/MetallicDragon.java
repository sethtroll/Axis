...
public final class MetallicDragon extends NPC implements CombatScript, Spawnable {
    ...
    public int attack(final Entity target) {
        ...
        if (style == 0 && target instanceof Player player) {
            setAnimation(DRAGONFIRE_ANIM);
            final Dragonfire dragonfire = new Dragonfire(DragonfireType.CHROMATIC_DRAGONFIRE, 50, DragonfireProtection.getProtection(player));
            delayHit(World.sendProjectile(this, target, DRAGONFIRE_PROJ), target, new Hit(this, Utils.random(Math.max(0, dragonfire.getDamage())), HitType.REGULAR).onLand(hit -> {
