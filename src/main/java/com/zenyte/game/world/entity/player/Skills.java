...
public final class Skills {
    ...
    public void addXp(final int skill, double exp, final boolean additionalBoost, final boolean fake) {
        ...
        //Checking for xp perks
        PerkWrapper key = Skills.isCombatSkill(skill) ? PerkWrapper.COMBAT_XP : PerkWrapper.SKILLING_XP;
        Perk xpPerk = player.getPerkManager().get(key);
        if (xpPerk != null && xpPerk.valid()) {
            exp *= xpPerk.getEffect();
        }
